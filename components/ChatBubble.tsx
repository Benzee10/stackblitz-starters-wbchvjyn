...
const ChatBubble: React.FC<ChatBubbleProps> = ({ message }) => {
  ...
  return (
    <div className={`flex w-full mb-3 ${isMe ? 'justify-end' : 'justify-start items-start'}`}>
      {!isMe && (
        <div className="w-8 h-8 bg-[#FFFC00] rounded-full mr-3 flex-shrink-0" />
      )}
      <div className={`relative max-w-xs md:max-w-md px-3 py-2 rounded-lg shadow-sm ${isMe ? 'bg-[#FFFC00]' : 'bg-gray-800'}`}>
        {!isMe && message.senderName && (
          <div className={`text-sm font-bold mb-1 ${message.senderColor || 'text-white'}`}>
            {message.senderName}
          </div>
        )}
        <div className={`text-sm break-words ${isMe ? 'text-black' : 'text-white'}`}>
          {message.text}
        </div>
        <div className={`text-right text-xs mt-1 flex items-center justify-end ${isMe ? 'text-gray-600' : 'text-gray-400'}`}>
